...
import { tokenFromAuthMetadata } from '../dist/guards/oauth.guard.js';
...
test('MCP OAuth guard extracts Bearer tokens case-insensitively like the HTTP boundary', () => {
  assert.equal(tokenFromAuthMetadata({ authorization: 'Bearer test-token' }), 'test-token');
  assert.equal(tokenFromAuthMetadata({ authorization: 'bearer test-token' }), 'test-token');
  assert.equal(tokenFromAuthMetadata({ authorization: 'BEARER test-token' }), 'test-token');
  assert.equal(tokenFromAuthMetadata({ authorization: 'Basic test-token' }), undefined);
  assert.equal(tokenFromAuthMetadata({}), undefined);
});
