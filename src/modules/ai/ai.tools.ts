...
export class AITools {
  ...
  @RateLimit({ requests: 10, window: '1m' })
  @Widget('composer')
  async improvePost(input: z.infer<typeof improvePostSchema>, _ctx: ExecutionContext) { return this.run(input, 'post improvement', improveOutput, `Improve the post for ${input.goal ?? 'clarity and usefulness'} and audience ${input.audience ?? 'the intended X audience'}. Keep the improved draft at most 280 characters. ${input.suggestHashtags ? 'Suggest hashtags only when relevant.' : 'Do not suggest hashtags.'} Preserve factual claims and state limitations.`, [userSource('original post', input.text), ...(input.context ? [userSource('additional context', input.context)] : [])]); }
  ...
  async summarizeThread(input: z.infer<typeof threadInputSchema>, _ctx: ExecutionContext) { return this.xGrounded(input, 'thread summary', summaryOutput, 'Summarize only the retrieved thread. Identify key points, participants, important claims, unresolved points, and incomplete/truncated data.', async () => [{ sourceType: 'x_api', label: 'real X thread', data: await this.x.getThread(input.postId) }]); }
